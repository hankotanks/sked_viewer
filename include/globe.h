...
void Globe_configure_buffers(Globe mesh, GLuint VA_id, GLuint* VAO, GLuint* VBO, GLuint* EBO) {
    glGenVertexArrays(1, VAO);
    glGenBuffers(1, VBO);
    glGenBuffers(1, EBO);
    glBindVertexArray(*VAO);
    glBindBuffer(GL_ARRAY_BUFFER, *VBO);
    glBufferData(GL_ARRAY_BUFFER, mesh.vertex_buffer_size, mesh.vertices, GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, *EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, mesh.index_buffer_size, mesh.indices, GL_STATIC_DRAW);
...
void Globe_draw(Globe mesh, GLuint VAO) {
    glBindVertexArray(VAO);
    glDrawElements(GL_TRIANGLES, mesh.index_count, GL_UNSIGNED_INT, (GLvoid*) 0);
}
...
Globe Globe_generate(GlobeProp cfg) {
    ...
    size_t vertex_count = (cfg.slices + 1) * (cfg.stacks + 1);
    GLsizei vertex_buffer_size = vertex_count * 3 * sizeof(GLfloat);
    GLfloat* vertices = (GLfloat*) malloc(vertex_buffer_size);
    if(vertices == NULL) return GLOBE_ERROR;
    size_t k = 0;
    vertices[k++] = (GLfloat) 0.f;
    vertices[k++] = (GLfloat) cfg.rad;
    vertices[k++] = (GLfloat) 0.f;
    for(size_t i = 0; i < (cfg.stacks - 1); ++i) {
        ...
        for(size_t j = 0; j < cfg.slices; ++j) {
            ...
            vertices[k++] = (GLfloat) (sinf(phi) * cosf(theta) * cfg.rad);
            vertices[k++] = (GLfloat) (cosf(phi) * cfg.rad);
            vertices[k++] = (GLfloat) (sinf(phi) * sinf(theta) * cfg.rad);
        }
    }
    vertices[k++] = (GLfloat) 0.f;
    vertices[k++] = (GLfloat) (cfg.rad * -1.f);
    vertices[k++] = (GLfloat) 0.f;
    size_t index_count = cfg.slices * 6 * (cfg.stacks - 1);
    GLsizei index_buffer_size = index_count * sizeof(GLuint);
    ...
    for(GLuint i = 0; i < slices_gl; ++i) {
        ...
        indices[k++] = (GLuint) (vertex_count - 1);
        indices[k++] = i0;
        indices[k++] = i1;
    }
    for(GLuint j = 0; j < (stacks_gl - 2); ++j) {
        ...
        for(GLuint i = 0; i < slices_gl; ++i) {
            ...
            GLuint i3 = j1 + i;
            indices[k++] = i3; indices[k++] = i0; indices[k++] = i1;
