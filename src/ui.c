...
float nk_win_height(struct nk_context* ctx, enum nk_panel_flags flags, size_t rows) {
    float height = 0.f;
    ...
    height += row_height_full * (float) rows;
...
void Overlay_prepare_interface(Overlay* ui, const OverlayFrameData ui_data, const RGFW_window* const win) {
    const float row_height = nk_row_height(ui->ctx);
    //
    // top banner panel
    const enum nk_panel_flags banner_flags = NK_WINDOW_NO_SCROLLBAR | NK_WINDOW_NO_INPUT;
    const float banner_height = nk_win_height(ui->ctx, 0, 1);
    ...
    //
    // info & control panel
    const enum nk_panel_flags info_flags = NK_WINDOW_BORDER | NK_WINDOW_TITLE | NK_WINDOW_MINIMIZABLE;
    const float info_height = nk_win_height(ui->ctx, info_flags, 2);
    const struct nk_rect info_bounds = nk_rect(0.f, banner_height, (float) win->r.w * 0.3f, info_height);
